...
## 2. 共通スタイル（毎回このブロックを先頭に貼る）

英語のほうが画風が安定するので、プロンプト本体は英語で書く。
...
ファイル名・サイズ・背景の扱いは表のとおり。
**書き出しサイズは正方形1024×1024**（表紙だけ 1024×1536 の縦長）。

| # | ファイル名 | サイズ | 背景 | 用途 |
| --- | --- | --- | --- | --- |
| 1 | `cover-default.png` | 1024×1536 | コーラル塗り | しおりの既定表紙 |
| 2 | `empty-suitcase.png` | 1024×1024 | 透明 | 空の画面・初回紹介1枚目 |
| 3 | `loading-map.png` | 1024×1024 | 透明 | 旅程作成中・行き先ゼロ |
| 4 | `packed-done.png` | 1024×1024 | 透明 | 持ち物すべて完了 |
| 5 | `avatar-01.png` | 1024×1024 | 透明 | アバター（男性） |
| 6 | `avatar-02.png` | 1024×1024 | 透明 | アバター（女性） |
| 7 | `spot-bench-01.png` | 1024×1024 | 透明 | 空き時間チップ |
| 8 | `spot-bench-02.png` | 1024×1024 | 透明 | 空き時間チップ |
| 9 | `icon-home.png` | 1024×1024 | 透明 | 出発地アイコン |
| 10 | `icon-bed.png` | 1024×1024 | 透明 | 宿泊アイコン |

透明背景の素材も、**まずコーラル地で描かせてから背景を抜く**（§4）ほうが線と色が安定する。
GPT Image が透過 PNG に対応している場合は、共通スタイルの最終行を
...
Note: this must read as a clearly different person from the male avatar —
rose red hair and a wide-brim hat versus short black hair and a bucket hat.
```

### 7. `spot-bench-01.png` — 空き時間（男性・ひと休み）
...
### 8. `spot-bench-02.png` — 空き時間（女性・ひと休み）
...
10点をばらばらに作ると絵柄が揃わないので、**同じチャットスレッドで続けて生成**し、
2枚目以降は「Same style, same palette, same line weight as the previous image.」を
先頭に添えると顔つきが揃いやすい。
