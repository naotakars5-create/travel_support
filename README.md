...
| `EXPO_PUBLIC_RAKUTEN_AFFILIATE_ID` | - | 楽天アフィリエイトID。設定すると「泊まりなのに宿が未登録」のときだけ宿探しカード（PR表記つき）が出る。未設定なら出ない。 |
...
- **広告（送客リンク）**: 外部の広告ネットワーク（AdMob 等）ではなく、自前のアフィリエイトリンクで出す。AdMob は Expo Go では動かず Development Build / EAS Build が要るうえ、このアプリは「行き先・日程・泊数・予算」を既に持っているので、ターゲティングを外注するより自分で送客したほうが単価が高い。決まりごとは `src/lib/ads.ts` に集約してある。
  - **枠は2つだけ** — 持ち物リストの未チェック項目に添える商品リンク（`packing-gear`）と、泊まりなのに宿が未登録のときだけ出る宿探しカード（`plan-lodging`）。**どちらも「条件を満たさなくなったら消える」**設計で、チェックを付ける・宿を1件登録するだけで無くなる。
...
- `lib/lodgingAd.ts` — 「泊まりなのに宿が未登録」の判定（`needsLodging`）と楽天トラベルの検索URL組み立て。日帰り・宿登録済み・旅行開始後はいずれも出さない。行き先の住所から都道府県を取り（`lib/region.ts`。しおりの表紙写真と同じ考え方）、楽天の地域コードへ変換してから検索する。**エンドポイントは `/ds/vacant/searchVacant`**（`/ds/yado/japan` に投げると日付もエリアも黙って無視され、既定の北海道の結果が出る／実機確認済み）。地域コードに変換できない行き先では枠ごと出さない — 無関係な県へ送客するより出さないほうが害が小さい。
