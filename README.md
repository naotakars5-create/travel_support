...
- `lib/furusatoAd.ts` — 旅のあとに出す、旅先の県のふるさと納税カード。**これは機能ではなく純粋な広告**（アフィリエイトを外したら消してよいUI）なので常設せず、しおり（旅が終わった旅）と当日画面（**最終日 かつ done**）の2か所に1枚ずつだけ出す。当日画面の例外が許されるのは、done では案内すべき「次」がそもそも存在せず、製品の核を傷つけないから。**move / free には絶対に出さない**。文言は年末（10〜12月）だけ締切を前に出し、それ以外の時期は急かさない。**税額・控除・「実質2,000円」には触れない**（限度額は年収や家族構成で変わり、誤解を招くと景表法・税務の両面で危うい）。
