...
| `EXPO_PUBLIC_RAKUTEN_AFFILIATE_ID` | - | 楽天アフィリエイトID。設定すると、宿泊先の枠の「まだ宿が決まっていない泊」と、「車の移動」枠のレンタカー未登録時に探すボタン（PR表記つき）が出る。未設定でも枠と手入力は使える。 |
...
- **AI提案の取り込み**: `/api/plan` が返すおすすめスポットは行き先リストに表示され、ワンタップで行き先リストへ追加できる。**宿だけ登録して観光が未定のときは、提案を自動で旅程へ組み込む**（`useAppState` の `composeWithAi`）。宿1件だけの旅程が返ってきても、ユーザーから見れば何も起きていないのと同じなので、この場合の提案は「候補」ではなく旅程の本体として扱う。AIにも「宿だけで観光が無い」ことと日数を伝え、1日3〜4件を挙げさせる。
- **宿泊中には予定を入れない**: チェックイン〜チェックアウトの間は、`allowDuringStay`（夜ご飯・夜景など）を除いて行き先を配置しない。**AIが割り当てた時刻にも効かせる**（`sequentialSchedule`）— 以前はアンカーがある場合に判定を素通りしていたため、泊まっている最中へ観光が差し込まれていた。ただし `fixedTime`（予約・便）はユーザーが決めた時刻なので動かさない。AIプロンプトには**チェックアウト時刻も渡す**（渡さないと「チェックイン〜チェックアウトの間に置かない」というルールを守りようがない）。
...
- **広告（送客リンク）**: 外部の広告ネットワーク（AdMob 等）ではなく、自前のアフィリエイトリンクで出す。AdMob は Expo Go では動かず Development Build / EAS Build が要るうえ、このアプリは「行き先・日程・泊数・予算」を既に持っているので、ターゲティングを外注するより自分で送客したほうが単価が高い。決まりごとは `src/lib/ads.ts` に集約してある。
  - **枠は5つだけ** — 持ち物リストの未チェック項目に添える商品リンク（`packing-gear`）、宿泊先の枠で「まだ宿が決まっていない泊」に出る宿探しボタン（`plan-lodging`）、「車の移動」枠でレンタカーが未登録のときに出る探すボタン（`plan-rental`）、旅が終わったしおりの末尾（`shiori-furusato`）と当日画面の最終日 done（`dayof-furusato`）に出るふるさと納税カード。**どちらも「条件を満たさなくなったら消える」**設計で、チェックを付ける・宿を登録する（または「この泊は宿を取らない」を選ぶ）だけで無くなる。
...
- `lib/furusatoAd.ts` — 旅のあとに出す、旅先の県のふるさと納税カード。遷移先は楽天ふるさと納税の県別ページ（`event.rakuten.co.jp/furusato/area/<都道府県コード>/`）で、コードは宿の `f_chu` と同じローマ字表記を使い回せる。楽天市場の検索でも返礼品は出せるが（返礼品は自治体ショップの楽天市場商品）、検索は無関係な商品が混ざりうるので**表示される中身を保証できる公式ページ**を使う。**これは機能ではなく純粋な広告**（アフィリエイトを外したら消してよいUI）なので常設せず、しおり（旅が終わった旅）と当日画面（**最終日 かつ done**）の2か所に1枚ずつだけ出す。当日画面の例外が許されるのは、done では案内すべき「次」がそもそも存在せず、製品の核を傷つけないから。**move / free には絶対に出さない**。文言は年末（10〜12月）だけ締切を前に出し、それ以外の時期は急かさない。**税額・控除・「実質2,000円」には触れない**（限度額は年収や家族構成で変わり、誤解を招くと景表法・税務の両面で危うい）。
- `lib/rentalAd.ts` — 「徒歩・電車が基本の旅でレンタカーが未登録」のときに出す探す導線。**`baseMode === "car"` は「ずっと車（マイカー）」＝自分の車なので勧めない**（読み違えやすい）。遷移先は**宿とは別サイト**で、`cars.travel.rakuten.co.jp/cars/rcf010a.do`（`travel.rakuten.co.jp/cars/search/` は404／実地確認済み）。都道府県コード（`gmarea`）は宿の `f_chu` と同じローマ字表記を使える。意味の分からないパラメータは実際の検索が発行した値をそのまま残している — 動く形からは不用意に削らない。
- `lib/region.ts` — 住所・行き先から都道府県を割り出す。県名の直接抽出（`prefectureOf` / `tripRegion`）に加え、市名・観光地名からの推定表（`prefectureFromArea`）を持つ。**曖昧な地名は表に入れないこと**（「府中」は東京と広島にある等）。推定を外して別の県の宿を出すより、出さないほうがよい。
